...
​		聚丙烯熔融指数与工业生产息息相关，而限于当下的技术水平，对聚丙烯熔融指数的实时测量又迟迟不能做到。在这种情况下，一款具有较高准确度的聚丙烯熔融指数数据实时预测软件的出现就能弥补聚丙烯工厂对其的刚性需求，因此这款程序完成后也具有广阔的市场前景。本程序将实现利用聚丙烯的九个参数值和已测得的聚丙烯熔融指数进行训练学习，从而得到拟合公式，再将测试值的参数进行拟合，得到拟合值。采用这种方法，能够脱离实验室直接测聚丙烯熔融指数的方法，比较好的解决聚丙烯熔融指数难以在线测、测不准的问题。
...
​		已知训练数据集$D=(X,y)=\{(X_i,y_i)|i=1,...,N\}$，其中为$X_i$输入向量，$y_i$为输出向量。现有新的输入$X^*$，预测对应的目标数据$\hat{y}^*$（真实输出为$y^*$）。

​		首先做出先验假设，即$f(X)$和$y^*$的联合概率分布：
$$
\left[ \begin{matrix} f(x)\\ y^* \end{matrix} \right] \sim \left[ 0， \left( \begin{matrix} K_{ff} & K_{fy}\\ K_{fy}^T & K_{yy} \end{matrix} \right) \right]
$$
​		其中$K_{ff} = k(x,x),K_{fy}=k(x,x^*),k_{yy}=k(x^*,x^*)$。根据多维高斯分布的条件分布性质可得：
$$
y^*|f \backsim N(k(x^*,x)k(x,x)^{-1}y,k(x^*,x^*)-k(x^*,x)k(x,x)^{-1}k(x,x*))
$$
​		再由高斯分布的概率分布可得，$\hat{y}^*=k(x^*,x)k(x,x)^{-1}y$的概率最大。

​		$k$是核函数(或协方差函数)，用来捕捉不同时刻随机变量之间的关系，有如下核函数可供选择：

![img](picture/3148087-cc06676702a155df.png)
...
​		使用sklearn.model_selection.train_test_split()函数来创建训练集和测试集，初始划分75%的数据为训练集，剩余25%的数据为验证集。
...
​		根据上一步结果的比较，初步决定使用高斯过程回归作为最终算法。在此基础上，还可以对数据集按不同的比例进行划分并比较训练结果。

​		若将数据集划分60%的数据为训练集，剩余40%的数据为验证集，得到的结果如下：

![image-20210120165738823](picture/image-20210120165738823.png)

​		![GPR2](picture/GPR2.png)

​		若将若将数据集划分70%的数据为训练集，剩余30%的数据为验证集，得到的结果如下：

![image-20210120165941049](picture/image-20210120165941049.png)

![GPR3](picture/GPR3.png)

​		若将数据集划分80%的数据为训练集，剩余20%的数据为验证集，得到的结果如下：

![image-20210120170121914](picture/image-20210120170121914.png)

![GPR4](picture/GPR4.png)

​		若将数据集划分90%的数据为训练集，剩余10%的数据为验证集，得到的结果如下：

![image-20210120170238810](picture/image-20210120170238810.png)

![GPR5](picture/GPR5.png)

​		可见高斯过程回归算法最终的拟合优度相差不大，都十分接近1，效果满足预期要求。
